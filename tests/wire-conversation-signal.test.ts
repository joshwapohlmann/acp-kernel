...
import { conversationSignalAnthropic, type AnthropicRequestBody } from "../src/wire/anthropic.js";
import { conversationSignalOpenai, type OpenAIRequestBody } from "../src/wire/openai.js";
...
test("conversationSignalAnthropic: bodies without a messages array fall back to the default fingerprint (#298)", () => {
    for (const raw of malformedBodies) {
        assert.doesNotThrow(() => conversationSignalAnthropic(raw as AnthropicRequestBody));
        assert.equal(conversationSignalAnthropic(raw as AnthropicRequestBody), DEFAULT);
    }
});

test("conversationSignalOpenai: bodies without a messages array fall back to the default fingerprint (#298)", () => {
    for (const raw of malformedBodies) {
        assert.doesNotThrow(() => conversationSignalOpenai(raw as OpenAIRequestBody));
...
test("conversationSignal*: non-user and null message elements never crash and fall back to default (#298)", () => {
    const anthropicBody = { messages: [null, { role: "assistant", content: "hi" }] } as unknown as AnthropicRequestBody;
    assert.equal(conversationSignalAnthropic(anthropicBody), DEFAULT);
    const openaiBody = { messages: [null, { role: "system", content: "sys" }] } as unknown as OpenAIRequestBody;
...
test("conversationSignalAnthropic: first user message content still drives the fingerprint", () => {
    const body = { messages: [{ role: "user", content: "hello" }, { role: "assistant", content: "x" }] } satisfies AnthropicRequestBody;
    assert.equal(conversationSignalAnthropic(body), hashId(JSON.stringify("hello")));
    const other = { ...body, messages: [{ role: "user", content: "world" }] } satisfies AnthropicRequestBody;
    assert.notEqual(conversationSignalAnthropic(other), conversationSignalAnthropic(body));
...
test("conversationSignalOpenai: first user message content still drives the fingerprint", () => {
    const body = { messages: [{ role: "user", content: "hello" }, { role: "assistant", content: "x" }] } satisfies OpenAIRequestBody;
    assert.equal(conversationSignalOpenai(body), hashId("hello"));
    const other = { ...body, messages: [{ role: "user", content: "world" }] } satisfies OpenAIRequestBody;
    assert.notEqual(conversationSignalOpenai(other), conversationSignalOpenai(body));
